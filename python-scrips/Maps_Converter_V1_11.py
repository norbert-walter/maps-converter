...
def map_demo():
    ...
    <head>
        <meta charset="UTF-8">
        <title>GPS & Kompass Beispiel</title>
        ...
        <style>
            ...
            #gps-data, #compass-data, #error-message {
                font-size: 1.2em;
                margin-top: 15px;
            }
            #map-image {
    ...
    <body>
        ...
        <img id="map-image" alt="Loading map...">
        ...
        <script>
            function updateMapImage() {
                if (navigator.geolocation) {
                    ...
                    document.getElementById('error-message').innerText = "Geo location not supported";
                }
            }
            ...
            function showPosition(position) {
                ...
                if (heading === null || isNaN(heading)) {
                    heading = 0.0; // Fall back valule in case of missing value
                }
                if (speed === null || isNaN(speed)) {
                    speed = 0.0; // Fall back valule in case of missing value
                }
                ...
                const imageUrl = `http://127.0.0.1:8080/get_image?zoom=15&lat=${lat}&lon=${lon}&mtype=5&mrot=${heading}&itype=4&dtype=1&width=400&height=800&debug=1&t=${timestamp}`;

                const img = document.getElementById('map-image');
                img.src = imageUrl;
                img.alt = `Karte für Position: ${lat}, ${lon}`;
            }
            
            function showError(error) {
                const messages = {
                    1: "Position sharing not allowed",
            ...
            window.addEventListener('load', () => {
                updateMapImage();
                setInterval(updateMapImage, 10000); // alle 10s GPS & Bild aktualisieren
            });
        </script>
    </body>
    </html>
